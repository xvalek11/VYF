...
typedef struct {
    float Rs,Gs,Bs,Ds,Rv,Gv,Bv,Dv,Rj,Gj,Bj,Dj,Rz,Gz,Bz,Dz;
} my_constraint_data1;

double myvfunc(const std::vector<double> &x, std::vector<double> &grad, void *my_func_data)
{
    my_constraint_data1 *d = reinterpret_cast<my_constraint_data1*>(my_func_data);
    float Rs = d->Rs, Gs = d->Gs, Bs = d->Bs,  Rj = d->Rj, Gj = d->Gj, Bj = d->Bj,  Rv = d->Rv, Gv = d->Gv, Bv = d->Bv,  Rz = d->Rz, Gz = d->Gz, Bz = d->Bz, Ds = d->Ds, Dv = d->Dv, Dj = d->Dj, Dz = d->Dz;
/*if (!grad.empty()) {
grad[0] = 0.0;
grad[1] = 0.5 / sqrt(x[1]);
}*/
...
void optimalize(float Rs, float Gs, float Bs, float Rv, float Gv, float Bv, float Rj, float Gj, float Bj, float Rz, float Gz, float Bz, float &Rres, float &Gres, float &Bres, float ds, float dv, float dj, float dz){
    //std::cout << "zde" << "\n";
    ...
    std::cout << "found minimum at f(" << x[0] << "," << x[1] <<  "," << x[2] << ") = " << std::setprecision(10) << minf << std::endl;
    Rres = x[0];
    Gres = x[1];
    Bres = x[2];
    //printf("End \n");
